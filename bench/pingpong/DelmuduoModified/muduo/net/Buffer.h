...
class Buffer {
   public:
    ...
    explicit Buffer(size_t initial_size = kInitialSize, size_t reserved_prepend_size = kCheapPrependSize)
        : capacity_(reserved_prepend_size + initial_size), read_index_(reserved_prepend_size), write_index_(reserved_prepend_size), reserved_prepend_size_(reserved_prepend_size) {
   ...
   private:
    ...
    void grow(size_t len) {
        if (writableBytes() + PrependableBytes() < len + reserved_prepend_size_) {
            ...
            memmove(begin() + reserved_prepend_size_, begin() + read_index_, readableBytes());
            read_index_ = reserved_prepend_size_;
